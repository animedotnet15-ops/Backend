...
from Backend.helper.modal import SettingsSchema, MovieSchema, TVShowSchema, MovieListSchema, TVShowListSchema, HomeSectionSchema, BrokenLinkReportIn, WatchlistIn, ReviewIn
...
# ─────────────────────────────────────────────────────────────────────────────
# Report Broken Link — sends straight to your Telegram REPORT_CHANNEL
# ─────────────────────────────────────────────────────────────────────────────
@app.post("/api/report")
async def report_broken_link(report: BrokenLinkReportIn):
    try:
        from Backend.helper.notification import send_broken_link_report
        await send_broken_link_report(report.dict())
        return {"status": "success"}
    except Exception as e:
...
# ─────────────────────────────────────────────────────────────────────────────
# Watchlist / My List — synced by owner_id (Firebase UID if logged in,
# otherwise 'guest:<device_id>' so it still works for logged-out users)
# ─────────────────────────────────────────────────────────────────────────────
@app.post("/api/watchlist")
async def add_to_watchlist(item: WatchlistIn):
    try:
        await db.add_to_watchlist(item.dict())
        return {"status": "success"}
    except Exception as e:
        LOGGER.error(f"Error adding to watchlist: {e}")
        raise HTTPException(status_code=400, detail=str(e))

@app.get("/api/watchlist/{owner_id}")
async def get_watchlist(owner_id: str):
    try:
        items = await db.get_watchlist(owner_id)
        return {"items": items}
    except Exception as e:
        LOGGER.error(f"Error fetching watchlist: {e}")
        raise HTTPException(status_code=400, detail=str(e))

@app.delete("/api/watchlist/{owner_id}/{media_type}/{tmdb_id}")
async def remove_from_watchlist(owner_id: str, media_type: str, tmdb_id: Union[int, str]):
    try:
        removed = await db.remove_from_watchlist(owner_id, tmdb_id, media_type)
...
# ─────────────────────────────────────────────────────────────────────────────
# Ratings & Reviews — one review per device per title (anonymous, device_id based)
# ─────────────────────────────────────────────────────────────────────────────
@app.post("/api/reviews")
async def submit_review(review: ReviewIn):
    try:
        await db.add_or_update_review(review.dict())
        return {"status": "success"}
    except Exception as e:
        LOGGER.error(f"Error submitting review: {e}")
        raise HTTPException(status_code=400, detail=str(e))

@app.get("/api/reviews/{media_type}/{tmdb_id}")
async def get_reviews(media_type: str, tmdb_id: Union[int, str]):
    try:
        data = await db.get_reviews(tmdb_id, media_type)
        return data
    except Exception as e:
        LOGGER.error(f"Error fetching reviews: {e}")
        raise HTTPException(status_code=400, detail=str(e))

@app.delete("/api/reviews/{media_type}/{tmdb_id}/{owner_id}")
async def delete_review(media_type: str, tmdb_id: Union[int, str], owner_id: str):
    try:
        removed = await db.delete_review(owner_id, tmdb_id, media_type)
        return {"status": "success" if removed else "not_found"}
    except Exception as e:
...
async def admin_list_reviewed_titles(admin: dict = Depends(verify_admin)):
    """Every title that has reviews, with count + average — for the admin dashboard."""
    try:
        titles = await db.get_reviewed_titles()
        return {"titles": titles}
    except Exception as e:
        LOGGER.error(f"Error fetching reviewed titles: {e}")
        raise HTTPException(status_code=400, detail=str(e))

@app.get("/api/admin/reviews/{media_type}/{tmdb_id}")
async def admin_get_reviews_for_title(media_type: str, tmdb_id: Union[int, str], admin: dict = Depends(verify_admin)):
    try:
        items = await db.get_reviews_admin(tmdb_id, media_type)
        return {"items": items}
    except Exception as e:
        LOGGER.error(f"Error fetching reviews: {e}")
        raise HTTPException(status_code=400, detail=str(e))
...
async def admin_delete_review(review_id: str, admin: dict = Depends(verify_admin)):
    try:
        removed = await db.admin_delete_review_by_id(review_id)
        return {"status": "success" if removed else "not_found"}
    except Exception as e:
        LOGGER.error(f"Error deleting review: {e}")
        raise HTTPException(status_code=400, detail=str(e))
...
async def resolve_episode_metadata(tmdb_id, season_num: int, episode_num: int):
    """
    Try to fetch the real episode title + thumbnail from TMDB for (season_num, episode_num).
    Some uploads use an absolute/continuous episode numbering scheme (e.g. Naruto S03E71)
    that doesn't match TMDB's actual per-season episode numbering. If the direct lookup
    fails, fall back to treating episode_num as an absolute count across all real seasons
    and re-map it to the correct (season, episode) pair before retrying.
    Returns (title, backdrop_url). Falls back to a generic title if nothing works.
    """
    fallback_title = f"Episode {episode_num}"
    fallback_backdrop = ""
    try:
        tmdb_id_int = int(tmdb_id)
    except (TypeError, ValueError):
        return fallback_title, fallback_backdrop

    # 1. Try the direct (season, episode) lookup first
    try:
        ep_details = await tmdb.episode(tmdb_id_int, season_num, episode_num).details()
        title = ep_details.name or fallback_title
        backdrop = (
            f"https://image.tmdb.org/t/p/original{ep_details.still_path}"
            if ep_details.still_path else fallback_backdrop
        )
        return title, backdrop
    except Exception as e:
        LOGGER.info(f"Direct TMDB lookup failed for S{season_num}E{episode_num} (tmdb_id={tmdb_id}): {e}")

    # 2. Fall back to absolute-numbering mapping using the show's real season structure
    try:
        show_details = await tmdb.tv(tmdb_id_int).details()
        seasons_list = [
            {
                "season_number": getattr(s, "season_number", 0),
                "episode_count": getattr(s, "episode_count", 0),
            }
            for s in getattr(show_details, "seasons", [])
        ]
        ...
        for s in regular_seasons:
            for r_num in range(1, s.get("episode_count", 0) + 1):
                abs_to_rel[current_abs] = (s.get("season_number"), r_num)
                current_abs += 1

        if episode_num in abs_to_rel:
            resolved_season, resolved_episode = abs_to_rel[episode_num]
            ep_details = await tmdb.episode(tmdb_id_int, resolved_season, resolved_episode).details()
            title = ep_details.name or fallback_title
            backdrop = (
                f"https://image.tmdb.org/t/p/original{ep_details.still_path}"
                if ep_details.still_path else fallback_backdrop
            ...
            LOGGER.info(
                f"Resolved absolute episode {episode_num} -> S{resolved_season}E{resolved_episode} for tmdb_id={tmdb_id}"
            )
            return title, backdrop
    except Exception as e:
        LOGGER.info(f"Absolute-mapping TMDB lookup also failed for E{episode_num} (tmdb_id={tmdb_id}): {e}")
...
async def admin_link_manual_file(payload: Dict[str, Any], send_report: bool = True, admin: dict = Depends(verify_admin)):
    ...
    try:
        ...
        if media_type == "movie":
            ...
            # Ensure Episode exists in that season (fetch real title/thumbnail from TMDB first,
            # with absolute-numbering fallback for shows like Naruto that don't use TMDB's
            # per-season episode numbers in their filenames)
            resolved_title, resolved_backdrop = await resolve_episode_metadata(tmdb_id, season_num, episode_num)
            await db.tv_collection.update_one(
                {
                ...
                {"$push": {"seasons.$.episodes": {
                    ...
                    "episode_backdrop": resolved_backdrop, 
                    "telegram": []
                }}}
            )
